...
int main(int argc, char** argv) {
    ...
    int remaining_rows = N % size; //Выдаём каждому процессу +1 строку (т.е. добиваем остаток через +1 каждому процессу)
    int local_rows = rank < remaining_rows ? rows_per_process + 1 : rows_per_process; //Если у нас не нулевой остаток
    ...
    // Синхронизация окна b
    MPI_Win_fence(0, win_b);

    // Каждый процесс получает вектор b
    ...
    // Синхронизация окна результата
    MPI_Win_fence(0, win_result);

    // Каждый процесс загружает свою часть результата
    int offset = 0;
    ...
    if (rank == 0) {
        ...
        std::copy(result_shared, result_shared + N, final_result.begin());
        std::cout << "Результирующий вектор:" << std::endl;
        for (float val : final_result) {
            std::cout << val << " ";
        }
        std::cout << std::endl;
    }
